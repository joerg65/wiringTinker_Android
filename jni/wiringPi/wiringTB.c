...
int tinker_board_setup(int rev)
{
	...
	for(i=0;i<9;i++)
    {
		...
		if (gpio_map0[i] < 0)
		{
			printf("wiringPiSetup:gpio_map: Unable to open /dev/mem: %s\n", strerror (errno));
			return -1;
		}
	...
	if (grf_map < 0)
	{
		printf("wiringPiSetup:grf_map: Unable to open /dev/mem: %s\n", strerror (errno));
		return -1;
	}
	...
	if (pwm_map < 0)
	{
		printf("wiringPiSetup:pwm_map: Unable to open /dev/mem: %s\n", strerror (errno));
		return -1;
	}
	...
	if (pmu_map < 0)
	{
		printf("wiringPiSetup:pmu_map: Unable to open /dev/mem: %s\n", strerror (errno));
		return -1;
	}
	...
	if (cru_map < 0)
	{
		printf("wiringPiSetup:cru_map: Unable to open /dev/mem: %s\n", strerror (errno));
		return -1;
	}
