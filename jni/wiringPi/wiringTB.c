...
int asus_get_pin_mode(int pin)
{
       ...
       if (func == GPIO)
       {
        ...
        printf("get_pin_mode: pin=%d, value=%x, func=%x\n",pin, value, func);
        return func;
}

void asus_set_pinmode_as_gpio(int pin)
{
        printf("asus_set_pinmode_as_gpio, pin = %d\n",pin);
        switch(pin)
        {
                //GPIO0
                case GPIO0_C1 :
...
void asus_set_pin_mode(int pin, int mode)
{
        ...
        printf("asus_set_pin_mode, pin = %d, mode = %d\n",pin, mode);

        if(!gpio_is_valid(pin))
                return;
        printf("asus_set_pin_mode, gpio_is_valid\n");
        bank = gpioToBank(pin);
        bank_pin = gpioToBankPin(pin);
...
void asus_digitalWrite(int pin, int value)
{
        ...
        if(!gpio_is_valid(pin))
                return;
        printf("asus_digitalWrite, gpio_is_valid\n");
        bank = gpioToBank(pin);
        bank_pin = gpioToBankPin(pin);
...
int asus_digitalRead(int pin)
{
        ...
        //printf("asus_digitalRead, pin = %d, value = %d\n",pin, value);
...
void asus_pullUpDnControl (int pin, int pud)
{
        ...
        printf("asus_pullUpDnControl, gpio_is_valid\n");
        bank = gpioToBank(pin);
        bank_pin = gpioToBankPin(pin);
...
void asus_set_pinAlt(int pin, int alt)
{
        ...
        if(!gpio_is_valid(pin))
                return;
        printf("asus_set_pinAlt, gpio_is_valid\n");
        bank = gpioToBank(pin);
        bank_pin = gpioToBankPin(pin);
